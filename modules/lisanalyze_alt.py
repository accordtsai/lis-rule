...
def analyze(file_name, lis_struct, time, args):
    ...
    # Unit conversion; disabled due to the IU not being defined in Pint
    #if args.convert:
    #    import pint
    ...
    #    if alt_val > lis_struct[time]["ALT"]["ref_high"]:
    #        event_time = time
    ...
    #        event_str = "ALT too high (current value {}; reference value {})".format(alt_val, lis_struct[time]["ALT"]["ref_high"])
    ...
    #        if event_time not in __event_dict[file_name].keys():
    #            __event_dict[file_name][event_time] = []
    #        event_str = "ALT too high (current value {}; reference value {})".format(alt_val, __alt_ul)
    #        __event_dict[file_name][event_time].append(event_str)
    #if "ref_low" in lis_struct[time]["ALT"].keys():
    #    if alt_val < lis_struct[time]["ALT"]["ref_low"]:
    #        event_time = time
    #        if file_name not in __event_dict.keys():
    #            __event_dict[file_name] = {}
    #        if event_time not in __event_dict[file_name].keys():
    #            __event_dict[file_name][event_time] = []
    #        event_str = "ALT too low (current value {}; reference value {})".format(alt_val, lis_struct[time]["ALT"]["ref_low"])
    #        __event_dict[file_name][event_time].append(event_str)
    #else:
    #    if args.warn:
    ...
    #        if event_time not in __event_dict[file_name].keys():
    #            __event_dict[file_name][event_time] = []
    #        event_str = "ALT too low (current value {}; reference value {})".format(alt_val, __alt_ll)
    #        __event_dict[file_name][event_time].append(event_str)
    ...
    if alt_val > 1000:
        ...
        if event_time not in __event_dict[file_name].keys():
            __event_dict[file_name][event_time] = []
        event_str = "ALT markedly elevated ({} ({})); consider ischemia, infection, toxicity".format(alt_val, lis_struct[time]["ALT"]["unit"])
        __event_dict[file_name][event_time].append(event_str)
    ...
    #        if event_time not in __event_dict[file_name].keys():
    #            __event_dict[file_name][event_time] = []
    #        event_str = "AST/ALT > 2 (AST: {}, ALT: {}, AST/ALT: {}); consider alcoholic cirrhosis".format(ast, alt_val, ast/alt_val)
    #        __event_dict[file_name][event_time].append(event_str)
